...
for i in range(0,18):
    ...
    # finding all the files in current directory that end with .DTA
    ...
    measurement_name = dtm_file.replace(".dtm","")
    ...
    def opt(choice):
        ...
        if choice == 1: # options={"maxiter":1,"disp": True}
            ...
            return scipy.optimize.minimize(to_opt, kla_estimate,method ="COBYLA",tol=1e-6).x
        elif choice == 3:
            return scipy.optimize.minimize(to_opt,kla_estimate,method ="Powell",tol=1e-6).x
        else:
            print(choice)
